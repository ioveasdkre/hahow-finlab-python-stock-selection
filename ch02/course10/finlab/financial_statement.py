...
def patch2019(df):
    ...
    refined_name = (
        df[(dfname, '會計項目')].str.split(" ").str[0].str.replace("　", "").apply(remove_english)
    )
...
def pack_htmls(year, season, directory):
    ...
    # 做清理
    ret = {
        'balance_sheet': clean(year, season, balance_sheet),
        'income_sheet': clean(year, season, income_sheet),
    ...
    pickle.dump(
        ret, open('data/financial_statement/pack' + str(year) + str(season) + '.pickle', 'wb')
    )
...
def combine(d):
    ...
    tnames = ['balance_sheet', 'cash_flows', 'income_sheet', 'income_sheet_cumulate']
...
def fill_season4(tbs):
    ...
    # 排序好並更新tbs
    income_sheet = (
        income_sheet.reset_index().sort_values(['stock_id', 'date']).set_index(['stock_id', 'date'])
    )
