...
class HistoryRecorder(object):
    ...
    def plot_equality(self):
        ...
        if len(self.history['date']) == 0:
            ...
            print(
                '只有一天：', self.history['date'][0], ' 您的總資產為', self.history['equality'][0]
            )
            print('不畫圖（兩天以上才會畫圖）')
            return
        print('權益曲線（資產歷史紀錄）')
    ...
    def __calculate_balance(self, date, position):
        ...
        v1 = (
            df[stock_list1]['證券代號'].map(lambda v: position[v]).astype(float)
            * df[stock_list1]['收盤價'].astype(float)
            * 1000
        ).sum()
        v2 = (
            df[stock_list2]['證券名稱'].map(lambda v: position[v]).astype(float)
            * df[stock_list2]['收盤價'].astype(float)
            * 1000
        ).sum()
        ret = v1 + v2
